defmodule AdvisorAi.Workers.EmailMonitorWorker do
  ...
  defp process_new_email(user, email) do
    ...
    # Check if we've already processed this email recently (within last 5 minutes)
    if already_processed_recently?(user.id, email_data.message_id) do
      Logger.info("📧 Email Monitor: Skipping already processed email #{email_data.message_id}")
      :ok
    else
      # Mark this email as processed
      mark_email_as_processed(user.id, email_data.message_id)
